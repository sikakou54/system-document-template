...
本節は、社員名簿管理システムの全APIに適用する共通設計を定義し、APIは通信境界に限定して認証情報・HTTP入力の受付・構文検証・単一業務モジュール呼出・HTTP結果への変換だけを行う。APIの存在・API-ID・主処理モジュールへの接続は[API一覧](05_API設計.md#52-api一覧)を構成上の正本とし、本章はHTTP契約と境界処理を詳細化する。
...
- API-001〜009およびAPI-011〜018は[API一覧](05_API設計.md#52-api一覧)で完全修飾したM-002の公開IFだけを1回呼ぶ。API-010はM-003/IF-01だけを1回呼ぶ。
