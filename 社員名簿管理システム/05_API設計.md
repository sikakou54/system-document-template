...
固定コード値の唯一の正本は[共通区分定義](02_機能要件.md#25-共通区分定義)とし、本表はAPI入出力で用いる表記の参照表である。APIは列挙外を`VALIDATION_ERROR`として受け付けず、M-002も永続化前に同じ列挙で再検証する。

| API項目名 | NULL | [共通区分定義](02_機能要件.md#25-共通区分定義)の許可値 |
|---|---|---|
...
固定ロールコードの正本は[共通区分定義](02_機能要件.md#25-共通区分定義)、操作権限・閲覧スコープの正本は[ユースケース](02_機能要件.md#23-ユースケース)（ロール別条件は各UC）とし、APIはその認可結果をM-002から受け取る。複数ロール時は操作権限・許可項目の和集合、閲覧スコープは`ALL > ORGANIZATION > SELF`の優先順で確定し、有効ロール0件はすべて拒否する。
...
| retirementTypeCode | body | string | No | 退職区分。未設定または権限による非返却時は省略。返却時は[共通区分定義](02_機能要件.md#25-共通区分定義)の許可値 |
...
| 6 | employmentTypeCode | 必須かつ[共通区分定義](02_機能要件.md#25-共通区分定義)（[社員固定コードAPI参照表](05_API設計.md#513-社員固定コードapi参照表) API参照表）の許可値 | VALIDATION_ERROR |
...
| 5 | employmentTypeCode | 指定時は[共通区分定義](02_機能要件.md#25-共通区分定義)（[社員固定コードAPI参照表](05_API設計.md#513-社員固定コードapi参照表) API参照表）の許可値 | VALIDATION_ERROR |
...
| 2 | 社員異動呼出 | モジュール呼出 | 在籍状態、異動日時点のマスター、上長社員の存在・本人との同一性・異動日時点の入社/退職状態・退職予定、期間整合、版数を検証し、異動日前日の所属終了、異動日以降に開始する既存将来所属の取消、新所属登録、履歴を一つの原子更新として行い、成功後に監査する処理を委譲する | M-002/IF-05 社員異動 | (1)の要求 | 新所属または業務エラー |
...
| 409 | MASTER_NOT_ACTIVE | M-002/IF-05 | 組織・役職が異動日時点で無効、または上長社員が不存在・異動対象社員本人・異動日時点で入社前・退職済み・退職日到来済み・退職予定ありのいずれか | 共通形式へ変換 |
...
| 3 | retirementTypeCode | 指定時は[共通区分定義](02_機能要件.md#25-共通区分定義)（[社員固定コードAPI参照表](05_API設計.md#513-社員固定コードapi参照表) API参照表）の許可値。省略時はNULL | VALIDATION_ERROR |
...
| `employmentType` | 雇用区分 | employmentTypeCode | [共通区分定義](02_機能要件.md#25-共通区分定義)の表示名（正社員、契約社員等）へ変換 |
| `status` | 在籍状態 | status | [共通区分定義](02_機能要件.md#25-共通区分定義)の表示名（在籍中 / 退職）へ変換 |
...
| canceledRetirementTypeCode | body | string | No | 取消した退職区分。予定に未設定だった場合は省略。返却時は[共通区分定義](02_機能要件.md#25-共通区分定義)の許可値 |
| version | body | integer | Yes | 更新後版数 |
