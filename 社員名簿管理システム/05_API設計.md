...
APIの存在、API-ID、主処理モジュールへの接続は§5.2 API一覧を構成上の正本とし、本章はHTTP契約と境界処理を詳細化する。
...
| 6 | employmentTypeCode | 必須かつ§2.4（§5.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 5 | employmentTypeCode | 指定時は§2.4（§5.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 3 | retirementTypeCode | 指定時は§2.4（§5.1.3 API参照表）の許可値。省略時はNULL | VALIDATION_ERROR |
