...
APIの存在、シーケンスごとのAPI-ID、M-001からの呼出順、主処理モジュールへの接続は§3.1・§3.1.1を構成上の正本とし、本章はHTTP契約と境界処理を詳細化する。
...
固定コード値の唯一の正本は§2.6とし、本表はAPI入出力で用いる表記の参照表である。APIは列挙外を`VALIDATION_ERROR`として受け付けず、M-002も永続化前に同じ列挙で再検証する。
...
固定ロールコード、操作権限、閲覧スコープの唯一の正本は§2.7とし、APIは次の認可結果をM-002から受け取る。複数ロール時は操作権限と許可項目の和集合、閲覧スコープは`ALL > ORGANIZATION > SELF`の優先順で決定し、有効ロールが0件の利用者はすべて拒否する。
...
| retirementTypeCode | body | string | No | 退職区分。未設定または権限による非返却時は省略。返却時は§2.6の許可値 |
...
| 6 | employmentTypeCode | 必須かつ§2.6（§6.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 5 | employmentTypeCode | 指定時は§2.6（§6.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 3 | retirementTypeCode | 指定時は§2.6（§6.1.3 API参照表）の許可値。省略時はNULL | VALIDATION_ERROR |
...
| `employmentType` | 雇用区分 | employmentTypeCode | §2.6の表示名（正社員、契約社員等）へ変換 |
| `status` | 在籍状態 | status | §2.6の表示名（在籍中 / 退職）へ変換 |
