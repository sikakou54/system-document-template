...
本章は、社員名簿管理システムの全画面(SCR-001〜SCR-011)を詳細設計レベルで定義する。画面の存在・ID・遷移は[画面一覧](03_画面設計.md#31-画面一覧)・[画面遷移](03_画面設計.md#32-画面遷移)を正本とし、各画面を同一の9サブセクション様式(基本情報・画面レイアウト・初期表示・画面項目・画面イベント・入力チェック・画面状態/表示制御・画面遷移・メッセージ一覧)で記載する。
...
| SCR-005 | API-005 | EVT-01 | POST `/api/employees/{employeeId}/assignments` | `ASSIGNMENT_PERIOD_CONFLICT` / `INVALID_TRANSFER_DATE` | [画面状態・表示制御](03_画面設計.md#377-画面状態表示制御) 期間重複 | MSG-33 | OK |
...
| 更新競合 | API-004が版数不一致 | 編集を無効化し再取得だけ活性 | MSG-23 | UC-007/SP-8 |
...
| 更新競合 | API-006が版数不一致 | 入力を保持し、社員詳細から最新情報を再取得 | MSG-89 | UC-004/SP-6 |
...
| 更新競合 | API-012が版数不一致 | 一覧を再取得するまで編集・更新を無効化 | MSG-94 | UC-010/SP-9 |
...
| 更新競合 | API-014が版数不一致 | 一覧を再取得するまで編集・更新を無効化 | MSG-97 | UC-011/SP-7 |
...
| システムエラー | トークン候補の生成またはログイン成功監査の記録に失敗 | 認証情報を破棄し再入力可 | MSG-75 | UC-005/SP-6 |
...
| 更新競合 | API-017が版数不一致 | 割当編集を無効化しAPI-016で最新情報を再取得 | MSG-101 | UC-012/SP-8 |
