...
- SQL-IDは `SQL-XXX` とし、1 IDに実行可能な1 Statementを定義する。ただし1つの原子更新を構成する複数Statementは、同一SQL-IDへ実行順の接尾辞 `-A` / `-B` / `-C` … を付した「構成文」として1 SQL-ID内に定義してよい。各構成文は独立して `prepare()` でき、構成文ごとに `?1` から欠番なく採番する。複数Statementの原子実行はSQLを連結せず、§5/§8のTX-IDに従ってM-006がPrepared Statement配列（単一SQL-IDの全構成文、または複数SQL-ID）を1回の `env.DB.batch()` へ渡す。§9.2のクエリ一覧はSQL-ID単位で計数し、各SQL-IDと構成文を対応するTX-IDへ紐付ける。
