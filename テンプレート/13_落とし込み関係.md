...
- 本表は代表経路で使うSQLのbind例示であり、照合の正本は§9とする（§13.2）。§9の全placeholderとM-006処理詳細を正逆照合し、`?1`〜`?100`だけ、最大ordinalまで欠番なし、全値定義済み、最大ordinal=bind定義数=`.bind()`引数数とする。名前付き`:name`/`@name`/`$name`と匿名`?`を実SQLに使わず、配列/object/`undefined`を直接bindしない。
