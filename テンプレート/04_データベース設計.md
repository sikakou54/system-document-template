...
<!-- テーブル・カラム・DDLの正本は本節、M-006が実行するランタイムSQL本文の正本は§8とする。D1 bindingへのアクセスはM-006だけに許可し、API・JOB・他モジュールからの直接・間接アクセスを禁止する。 -->
<!-- 業務データの論理構造(エンティティ・属性・関連)は§2.3 データモデル、DB要素名と接続境界は§4.1 採用DBMS・物理設計方針および§4.2 テーブル一覧を正本とし、本章でそれらのSQLite物理構造を詳細化する。M-006以外からD1へ到達する経路を追加しない。 -->
...
| Workers binding | binding名は `DB`。`env.DB`を受領・参照できるのはM-006だけとし、環境別database ID・preview IDは§11で管理する |
...
| 文字コード | UTF-8。文字列の業務正規化規則は§7、検索用正規化・照合規則は本節と§8で一致させる |
...
- 部分indexはWHERE述語、式indexは実際の式、複合indexは列順とASC/DESCを記載し、§8のWHERE/ORDER BYと一致させる。
...
<!-- 複数Statementを不可分にする全処理を列挙し、§7、§8のTX-IDと一致させる。 -->
...
- 原子実行境界のTX-IDはM-002が業務単位で宣言し、ログイン系のM-003は原子更新境界を宣言しない（M-003はデータ参照だけをM-006へ委譲し、複数Statementの不可分更新を持たない）。宣言主体を§7.3ルール10・§9.4と一致させる。
...
- DDL・固定データはD1 migrationとして版管理し、Workers起動時またはAPI/JOBから実行しない。環境別適用手順と適用履歴は§11で定義する。
...
| 原子実行境界 | TX-XXX |
| バインドplaceholder | §8で順序付き `?1`, `?2`, ... を使用 |
