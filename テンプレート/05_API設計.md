...
<!-- 本節は統合設計書「5. API設計」の詳細設計テンプレート。API一覧に登録した全APIについて、§5.x の個別APIブロックを複製して定義する。代表APIだけを詳細化してはならない -->
...
<!-- APIの存在、API-ID、M-001からの呼出順、主処理モジュールへの接続は[API一覧](05_API設計.md#52-api一覧)を構成上の正本とし、本章ではHTTP契約と境界処理を詳細化する -->
...
定義ルール:
- API境界は1つの業務モジュールの公開処理だけを1回呼び出す。複数モジュールの調整は業務モジュール内で行う。
...
| 認証 | ログインAPIを除き、認証トークンを `Authorization: Bearer {token}` で受け付ける |
| 認可 | API境界で認証主体を確定し、[ユースケース](02_機能要件.md#23-ユースケース)の各UCで定義した操作権限・閲覧スコープ・項目制御に対する最終認可は呼出先業務モジュールへ委譲する |
...
固定コード値の唯一の正本は[共通区分定義](02_機能要件.md#25-共通区分定義)とし、本表にはAPI項目名、NULL可否、[共通区分定義](02_機能要件.md#25-共通区分定義)で定義した許可値を転記して物理契約との対応を示す。API境界と呼出先業務モジュールの双方で列挙外を拒否する。
...
<!--
【6.1.4 認可・閲覧範囲のAPI適用】
定義内容: §2.3の各UCで定義した認可条件を、API利用条件・データスコープ・返却/更新項目へ対応づける。
定義する条件: ロール、所属、本人性または項目でAPI契約が変わる場合に必須。
定義ルール: 全ロールを列挙し、スコープ、利用可能API、対象条件、項目許可、複数ロール合成、ロールなしDENYを省略しない。
-->
### 5.1.4 認可・閲覧範囲のAPI適用

固定ロールコードの正本は[共通区分定義](02_機能要件.md#25-共通区分定義)、操作権限、閲覧スコープおよび項目許可の正本は[ユースケース](02_機能要件.md#23-ユースケース)の各ユースケースとする。API境界は認証主体を渡し、業務モジュールが次表と[ユースケース](02_機能要件.md#23-ユースケース)から最終認可結果を確定する。

| ロールコード | データ閲覧スコープ | API利用条件・項目制御 |
|---|---|---|
| `<ROLE_CODE>` | `ALL` / `ORGANIZATION` / `SELF` | <利用可能API、対象条件、返却項目または更新可能項目> |

- 複数ロール時の操作権限・項目許可の合成、スコープ優先順位、許可組織集合、ロールなし時のDENYを[ユースケース](02_機能要件.md#23-ユースケース)と同じ規則で明記する。
...
- 呼出モジュールは1 APIにつき1件・1回とし、定義済みのM-ID/IF-IDまで完全修飾する。
-->
## 5.2 API一覧
...
- 固定業務コードは§2.5を正本として許可値を列挙し、未知コードとロール非許可値を区別する。
- 認可は§2.3の各UC定義を正本とし、個別APIで許可ロール、対象スコープ、返却項目または更新可能項目を省略せず列挙する。複数ロール時の合成は業務モジュールへ委譲し、API境界だけで確定しない。
...
- 呼出先公開IFが宣言する全業務例外、API境界の全検証例外・認証例外・技術例外を§5.x.7へ正確に1回ずつ割り当てる。発生元のないエラー、未変換の公開IF例外、同じ条件の重複割当を禁止する。
- 各§5.x.7のエラーは、そのAPIを利用する全画面の§3.0.2で共通制御またはMSGへ割り当てられていることを逆引き確認する。
-->
## 5.x XXX API
...
<!-- API境界で行う構文・形式検証を定義する。存在、重複、状態遷移、権限範囲、文字列の業務正規化後検証等はモジュールへ委譲する。固定コードは[共通区分定義](02_機能要件.md#25-共通区分定義)、認可は[ユースケース](02_機能要件.md#23-ユースケース)の各UC定義、出力fieldコードは個別APIの対応表との一致を検証する -->
...
| 4 | 正常レスポンス生成 | 境界 | 成功結果を§5.x.3の項目と正常HTTPステータスへ変換する | - | (2)の成功結果 | 正常レスポンス |
| 5 | エラーレスポンス生成 | 境界 | 公開IFの業務例外または境界例外を§5.x.7の一意な行と共通エラー形式へ変換する | - | (2)のエラー結果または境界例外、traceId | エラーレスポンス |

### 5.x.7 エラー定義

| HTTP | エラーコード | 発生元 | 発生条件 | API境界の処理 |
|---:|---|---|---|---|
| 400 | VALIDATION_ERROR | API境界 | §5.x.4の検証違反 | 共通エラー形式へ変換する |
...
公開IFの例外一覧と本表を正逆照合し、該当しない404/409等の例示行は削除する。各エラーコードは[画面・API・ERR・MSG横断整合](03_画面設計.md#302-画面apierrmsg横断整合)で、利用画面の共通制御またはMSGへ対応づける。

<!-- API一覧の全件について、上記 §5.x.1〜§5.x.7 を反復する -->
