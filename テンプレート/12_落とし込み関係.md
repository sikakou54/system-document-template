...
<!-- 本節は0〜12章体系の「12. 設計の落とし込み関係」テンプレート。要求からCloudflare D1までの実現経路とM-006限定データアクセス境界を1枚で示す。 -->

<!--
【12. 設計の落とし込み関係】
定義内容: 代表要求を、要求→機能・UC→画面/Workers API/Workers JOB→業務モジュール→M-006→D1 Prepared Statement→Cloudflare D1の順で具体化する経路として図示する。
定義する条件: 9章のトレーサビリティマトリクスと10章のレビュー観点が作成済みで、代表経路を具体IDで示せる段階で作成する。
項目説明:
- 要求・機能・UC: 業務の目的、振る舞い、連携順序。
- 画面/API/JOB: オンライン操作と定期・非同期起動の境界。
- モジュール: 業務処理とデータ処理の公開IF。
- SQL・D1: M-006だけが扱う順序付きbind、D1実行、物理実装。
定義ルール:
- 実在する代表BRを選び、各段階のIDを9章と一致させる。
...
- [12.3 認可済み条件の伝播（認可がある場合）](#123-認可済み条件の伝播認可がある場合)
...
- M-006だけが[クエリ設計](08_クエリ設計.md#8-クエリ設計)の静的SQL-IDを `prepare()` し、論理入力を順序付き `?1`,`?2`,...へ固定順で `.bind(...)` して、指定された `first()` / `all()` / `raw()` / `run()` / `batch()` を実行する。
- SQLの入力・出力・競合・性能・D1実行は[クエリ設計](08_クエリ設計.md#8-クエリ設計)、SQLを実行するM-006公開IFは[モジュール設計](07_モジュール設計.md#7-モジュール設計)、物理型・制約・trigger・FTS5は[データベース設計](04_データベース設計.md#4-データベース設計)を正本とする。
- 実装基盤・運用環境・容量・試験実装の未確定事項は11章へ引き継ぐ。
...
- 1つのTX-IDを複数M-006公開IFの逐次呼出で組み立てない。[データベース設計](04_データベース設計.md#4-データベース設計)、[モジュール設計](07_モジュール設計.md#7-モジュール設計)、[クエリ設計](08_クエリ設計.md#8-クエリ設計)、[要求・設計トレーサビリティ](09_トレーサビリティ.md#9-要求設計トレーサビリティ)でM-006 IF、SQL順、成功条件、全体失敗条件を一致させる。
...
- 本表は代表経路で使うSQLのbind例示であり、照合の正本は[クエリ設計](08_クエリ設計.md#8-クエリ設計)とする（[データアクセス境界](12_落とし込み関係.md#122-データアクセス境界)）。[クエリ設計](08_クエリ設計.md#8-クエリ設計)の全placeholderとM-006処理詳細を正逆照合し、`?1`〜`?100`だけ、最大ordinalまで欠番なし、全値定義済み、最大ordinal=bind定義数=`.bind()`引数数とする。名前付き`:name`/`@name`/`$name`と匿名`?`を実SQLに使わず、配列/object/`undefined`を直接bindしない。
...
[JOB設計](06_JOB設計.md#6-job設計)、[モジュール設計](07_モジュール設計.md#7-モジュール設計)、[JOB継続チェーン逆引き確認](09_トレーサビリティ.md#95-job継続チェーン逆引き確認)、[設計レビュー用チェックリスト](10_レビューチェックリスト.md#10-設計レビュー用チェックリスト)、[詳細設計への引継ぎ事項](11_詳細設計への引継ぎ.md#11-詳細設計への引継ぎ事項)と本節で、message項目、40件、900/1,000 Statement、Queue設定、retry/DLQ規則を一致させる。
