...
- M-006だけが§8の静的SQL-IDを `prepare()` し、論理入力を順序付き `?1`,`?2`,...へ固定順で `.bind(...)` して、指定された `first()` / `all()` / `raw()` / `run()` / `batch()` を実行する。
- SQLの入力・出力・競合・性能・D1実行は§8、SQLを実行するM-006公開IFは§7、物理型・制約・trigger・FTS5は§4を正本とする。
...
- 1つのTX-IDを複数M-006公開IFの逐次呼出で組み立てない。§4、§7、§8、§9でM-006 IF、SQL順、成功条件、全体失敗条件を一致させる。
...
- 本表は代表経路で使うSQLのbind例示であり、照合の正本は§8とする（§12.2）。§8の全placeholderとM-006処理詳細を正逆照合し、`?1`〜`?100`だけ、最大ordinalまで欠番なし、全値定義済み、最大ordinal=bind定義数=`.bind()`引数数とする。名前付き`:name`/`@name`/`$name`と匿名`?`を実SQLに使わず、配列/object/`undefined`を直接bindしない。
...
§6、§7、§9.5、§10、§11と本節で、message項目、40件、900/1,000 Statement、Queue設定、retry/DLQ規則を一致させる。
