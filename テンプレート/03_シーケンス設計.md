...
定義する条件: 複数要素の連携・分岐・D1原子更新を伴うUCで必須。M-001内で完結し画面設計で表現できる静的表示・単純操作は省略可(§3.1.1に省略理由を記す)。API経由でD1へ永続化するCRUDは、画面内で完結しないため省略しない(§3.1.1で「必要」と判定する)。
項目説明:
...
- 3.2〜 各シーケンス: 正常系・代替/例外系を Mermaid sequenceDiagram で示し、直後に連携定義を表で補足する。補足表はシーケンス種別により異なり、正常系・参照系・更新系は条件分岐・データ参照/更新・D1原子実行境界(参照のみは理由付きで「なし」)、代替・例外系は状態パターン対応、期間整合更新はさらに補足事項、定期・非同期JOBは固定設計表を用いる。
定義ルール:
- 各図の直後の条件分岐「根拠」列に、対象UCの状態パターンを完全修飾(UC-XXX/SP-x)で示し、正常/代替/例外の分岐と双方向に網羅する(SP-x の定義は §2.2 が正本)。ただしversion競合・batch全体ロールバック等、§2.2の業務状態パターンに直接対応しない技術的判定については、根拠列に関連するNFR-ID・実装する公開IF・TX-IDを示してよい。
- 条件分岐表の条件ID(COND-XX)は各図ローカルの連番であり、図をまたいで同一IDが同一条件を指すことは保証しない。他節から参照する場合は §3.x/COND-XX と完全修飾する。D1原子実行境界表のTX-IDは §5 データベース設計 を採番・正本とし(§3・§8・§9のTX-IDと一致させる)、本節では §5 で定義済みのTX-IDに一致させて用いる。
...
- §3.2〜§3.6は正常系・代替/例外系・参照系・期間整合更新・定期非同期JOBの類型見本であり、§3.1.1で「必要」と判定した各UCについて該当する類型の節を§3.2から連番で複製する(§3.2・§3.3 …)。複数UCが必要な場合もUC単位で節ブロックを追加して節番号を通しで採番し、対象UCに存在しない類型の節は設けない。
-->
# 3. シーケンス設計
...
- 各状態パターン(UC-XXX/SP-x)は 正常系 / 代替・例外系 のいずれかとして、各図中のalt分岐または直後の連携定義(条件分岐表)で表現し、各シーケンス直後の連携定義(条件分岐)の根拠列に完全修飾で紐付ける。図中へのalt分岐描画は類型に応じて選択でき、必須としない。
-->

本節は、§3.1.1で図が必要と判定されたユースケース XXX の連携(利用者・画面・Workers上のAPI/JOB・機能・M-006・Cloudflare D1・監査)を時系列に検証する。各状態パターンは正常系または代替・例外系のいずれかとして、各図中のalt分岐または直後の連携定義(条件分岐表)で表現し、各図の直後の連携定義でデータ参照・更新とD1原子実行境界を補足する。
...
- 種別: アクター / 画面 / 画面グループ / DB / API / 起動基盤 / JOBハンドラー / メッセージ基盤 / JOB / モジュール / 外部システム / クライアント状態。画面グループは複数のSCRを1つの参加者に束ねて表現する場合、クライアント状態はオフライン保持・ローカルキャッシュ等のクライアント側状態を参加者として明示する場合にだけ用い、いずれも該当シーケンスがなければ本表に登録しない。
...
- 行はアクター、画面(§4)、DB(§5)、API(§6)、JOB群(§7:起動基盤・JOBハンドラー・メッセージ基盤・JOB本体)、モジュール(§8)、外部システム・クライアント状態の順に、本表を正本として詳細化する章の順で並べる。JOB群の内部はCron Trigger→scheduledハンドラー→Cloudflare Queues→queueハンドラー→JOB本体の起動順に並べる。
...
<!--
【3.1.3 D1共通再試行契約】
定義内容: D1書込みで即時再試行してよい一時エラーの許可リスト、再試行回数・待機方式、書込結果不明時の状態/version再読込による成功・未反映・競合の判定、再試行しない条件を、全更新系シーケンス共通の契約として一度だけ定義する。
定義する条件: D1 batchによる更新系シーケンスを持つシステムで定義する(参照専用システムでは設けない)。許可リスト・再試行回数・待機時間の具体値は§7 JOB設計・§8 モジュール設計と完全一致させ、本節へ重複定義しない。
定義ルール:
- 即時再試行の対象は、基盤が再試行推奨として公開する一時エラーだけに限定する。許可リストは本節と§7 JOB設計で同一集合を完全一致で列挙し、分類・判定はM-006・M-002(§8)を正本とする。
- 再試行回数・待機(full jitter等)、書込結果不明時に状態/versionを読み直して成功・未反映・競合を判定する手順、再試行しない条件(version競合・制約違反・業務条件不成立)を示す。
- 過負荷・timeout・CPU・memory・実行文予算超過は即時再試行せず、オンラインAPIは一時障害応答、JOBはretryable属性付きエラーとしてQueue再配信へ委ねることを示す。
-->
### 3.1.3 D1共通再試行契約

D1書込みの即時再試行は、基盤が再試行推奨として公開する一時エラー(許可リストは本節と§7 JOB設計で完全一致、分類・判定はM-006・M-002を正本とする)だけを対象とし、M-002が初回に加えて最大N回、full jitterで待機してbatch全体を再試行する。書込み送信後に成否が不明な場合は、再試行前にM-006経由で対象状態とversionを読み直し、意図した状態・versionなら成功、事前状態なら未反映として再試行、どちらでもなければ競合とする。version競合・制約違反・業務条件不成立は再試行しない。過負荷・timeout・CPU・memory・実行文予算超過は即時再試行せず、オンラインAPIでは一時障害応答、JOBではretryable属性付きエラーとしてQueue再配信へ委ねる。
...
- 連携定義: 条件分岐 / データ参照・更新 / D1原子実行境界 を小表で補足する。条件分岐は 条件ID(COND-XX、図ローカル連番) / 判定箇所(判定するモジュールを§3.1の図中表示名またはM-IDで記す) / 条件 / 成立時 / 不成立時 / 根拠(UC-XXX/SP-x で完全修飾) の各列、データ参照・更新は エンティティ(§2.3の論理名) / CRUD / 目的 / 実行主体(§3.1の図中表示名。D1アクセスは「M-006 D1データアクセス」) の各列で記す。
定義ルール:
...
定義ルール:
- 1つの状態パターンに1つの分岐を対応させ、束ね表現(「または」で複数条件を1分岐)を避ける。保存中の技術的異常(TX全体ロールバック等)は§2.2の業務状態パターンに対応しないため、状態パターン列に業務パターン対象外である旨と関連TX-IDを記してよい。
