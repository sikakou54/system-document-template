...
- [8.2 クエリ一覧](#82-クエリ一覧)
- [8.3 SQL-XXX <クエリ名>](#83-sql-xxx-クエリ名)
- [8.X D1 batch対応表](#8x-d1-batch対応表)
- [8.X 検索方式補足](#8x-検索方式補足)

## 8.1 クエリ設計方針

<!-- 必須。D1 Prepared Statement、順序付きbinding、結果・例外・原子実行・性能の共通原則を定義する。 -->

- SQLは[データベース設計](04_データベース設計.md#4-データベース設計)のCloudflare D1 / SQLite方言、物理名、型、日時、制約、index、triggerに従う。
- SQL-IDは `SQL-XXX` とし、1 IDに実行可能な1 Statementを定義する。ただし1つの原子更新を構成する複数Statementは、同一SQL-IDへ実行順の接尾辞 `-A` / `-B` / `-C` … を付した「構成文」として1 SQL-ID内に定義してよい。各構成文は独立して `prepare()` でき、構成文ごとに `?1` から欠番なく採番する。複数Statementの原子実行はSQLを連結せず、[データベース設計](04_データベース設計.md#4-データベース設計)/[モジュール設計](07_モジュール設計.md#7-モジュール設計)のTX-IDに従ってM-006がPrepared Statement配列（単一SQL-IDの全構成文、または複数SQL-ID）を1回の `env.DB.batch()` へ渡す。[クエリ一覧](08_クエリ設計.md#82-クエリ一覧)はSQL-ID単位で計数し、各SQL-IDと構成文を対応するTX-IDへ紐付ける。
...
- 一覧と個別定義、[データベース設計](04_データベース設計.md#4-データベース設計)のTBL/IDX/constraint、[モジュール設計](07_モジュール設計.md#7-モジュール設計)のM-006 IF/TX-ID、[要求・設計トレーサビリティ](09_トレーサビリティ.md#9-要求設計トレーサビリティ)のトレースを正逆照合し、未使用SQLと未定義SQL参照を残さない。
...
| 追加確認対象 | SQL本文100 KB、bound string/BLOBまたは1 row 2 MB、SQL/batch実行30秒、LIKE/GLOB pattern 50 byteの現行D1上限を[詳細設計への引継ぎ事項](11_詳細設計への引継ぎ.md#11-詳細設計への引継ぎ事項)で確認日付き管理する |

Statement数の見積りだけで合格とせず、M-006がD1実行のたびに実測しM-002へ返す。上限値は2026-07-14時点のCloudflare公式資料を基準とし、plan・platform変更時は[詳細設計への引継ぎ事項](11_詳細設計への引継ぎ.md#11-詳細設計への引継ぎ事項)の確認記録を更新してから本節を変更管理する。
...
| 0行 | SQL固有の0件条件 | 空 / NOT_FOUND / UPDATE_CONFLICT | [モジュール設計](07_モジュール設計.md#7-モジュール設計)の公開IF契約に従う |
| 複数行 | 1行期待に違反 | DATA_INTEGRITY_ERROR | 生データを返さない |
| `meta.changes=1`等 | 期待変更件数一致 | DB確定日時・version等 | 正常更新 |
| constraint/trigger失敗 | 制約ID/安定コードを識別 | <DUPLICATE/CONFLICT等> | [モジュール設計](07_モジュール設計.md#7-モジュール設計).Y例外変換に従う |
...
<!-- [データベース設計](04_データベース設計.md#4-データベース設計)・[モジュール設計](07_モジュール設計.md#7-モジュール設計)の全TX-IDを1行ずつ定義し、Statement順とbind済みSQLを固定する。 -->
...
batch成功後の判定で全体ロールバックできると仮定しない。期待件数不一致でbatch全体を中止すべき場合は、同一batch内でSQLiteエラーを発生させるconstraint/trigger/guard Statementを[データベース設計](04_データベース設計.md#4-データベース設計)と本節に実行可能な形で定義する。成功した `changes=0` の事後判定だけでは原子性を保証しない。
...
FTS5 query構文として許可する入力、特殊文字の扱い、tokenizer、権限フィルター、content同期、安定順、最大件数を[データベース設計](04_データベース設計.md#4-データベース設計)のFTS-IDと同期する。採用時はSQL-IDを発行して[SQL-XXX <クエリ名>](08_クエリ設計.md#83-sql-xxx-クエリ名)形式で完全定義する。採用しない場合は本項を削除し、代替検索方式を記載する。
