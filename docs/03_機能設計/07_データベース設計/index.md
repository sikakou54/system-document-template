...
- 作成・更新前に `../11_トレーサビリティ/シーケンス設計トレーサビリティ.md` で関連SEQを特定し、概念MDLとSEQの参照更新・状態変更・トランザクション境界から属性・関連・整合性制約を物理化する。
...
| TBL-010 | 外部イベント受信記録トランザクション | T_INBOUND_EVENTS | 外部サービスの契約・請求イベントを冪等に受信記録する(Stripe Webhook Inbox。MDL-011) |
