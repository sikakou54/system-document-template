...
MeetRoom は Cloudflare 上に構築する会議室予約システムである。

- フロント(SPA)を Cloudflare Pages で実行する。
- REST API を Cloudflare Workers で実行する。
- 業務データを Cloudflare D1(SQLite)で実行する。
- セッション等の一時データを Cloudflare KV で実行する。
- 定期・非同期処理を Cloudflare Cron Triggers + Queues で実行する。
- メール送信を Resend と連携する。
- 有料会議室の従量課金を Stripe と連携する。
...
構成図に登場する各構成要素の役割と、詳細仕様の正本となる設計ドキュメントを示す。業務能力、データWriter、依存規則、トランザクション主体は [03_コンポーネント責務.md](03_コンポーネント責務.md) を正本とする。
...
| Cloudflare KV | データ | API-COM、03_コンポーネント責務.md | API冪等受付、レート制御、および失効方式を採用する場合のJWT失効情報をTTL付きで保持する。業務データの正本は保持しない |
...
| 初期ユーザー登録 | 本システムはアプリ上のユーザー登録(サインアップ)機能を持たない(スコープ外)。利用者(ユーザーマスタ)は、運用手順により MOD-008 で定義したハッシュ方式のパスワードを含むレコードを D1 へ投入(シード)して用意する。ログイン(CFR-001/UC-01)の前提条件はこの投入により満たす | CFR-001、TBL-001、MOD-008 |

# 6. 上流契約

- コンポーネント責務・データ所有・依存方向は [03_コンポーネント責務.md](03_コンポーネント責務.md) を正本とする。
- API、JOB/Queue、外部IFの論理契約は [04_論理インターフェース契約.md](04_論理インターフェース契約.md) を正本とする。
- シーケンス設計は上記契約を入力に、失敗・同時実行・再配送・ロールバックを検証する。
