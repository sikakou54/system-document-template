...
Stripe Webhook の反映(署名検証・状態反映・冪等処理)に失敗した場合のエラーレスポンスを返却する。封筒構造は API-COM §4 エラーレスポンスが正本。
...
- 署名の真正性の検証は §5 個別処理フロー((1) Webhook反映)に委譲する。

| 項目名 | 成立条件 | エラーコード |
|---|---|---|
| 署名 | 指定あり AND string | [ERR-006](エラーメッセージ一覧.md) |

# 7. エラー

本 API が返却するエラーの一覧。定義(エラー名・HTTPステータス・開発者向けメッセージ)は エラーメッセージ一覧.md が正本。本 API は署名検証で真正性を担保するため認証・認可の共通エラー(ERR-001 / ERR-002)は発生しない。発生条件は、共通エラー(署名ヘッダ必須検証の ERR-006)は API-COM §7 共通処理フロー、固有エラーは §4/§5 個別処理フローで表現する。

| エラーコード | 区分 | 発生箇所 |
|---|---|---|
| ERR-006 | 共通 | 共通処理フロー(入力バリデーション。署名ヘッダ必須検証) |
