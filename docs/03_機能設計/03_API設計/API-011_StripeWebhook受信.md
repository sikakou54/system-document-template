...
入力バリデーションで発生する共通エラーは API-COM_共通設計.md §4.1 共通エラー一覧を参照する。

- 本 API に適用される共通エラーは ERR-006(バリデーションエラー。§6 の署名ヘッダ必須検証)。
- 本 API は署名検証で真正性を担保するため認証・認可の共通エラー(ERR-001 / ERR-002)は発生しない。
...
|---|---|---|---|
| ERR-009 | 課金処理エラー | 500 | Billing processing error |
