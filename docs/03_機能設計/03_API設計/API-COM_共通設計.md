...
ERR-XXX の定義はカタログにまとめず各文書へインラインする。全 API 共通の ERR-001 / ERR-002 / ERR-006 は本書 §4.1 共通エラー一覧を正本とし、各 API 固有のエラーは当該 API 文書 §7 エラーに「ERR ID / エラー名 / HTTPステータス / 開発者向けメッセージ」の表でインライン定義する。エラーの発生条件は表に持たせず、共通エラーは §7 共通処理フロー、API 固有エラーは各 API 文書 §4/§5 個別処理フローで表現する。
...
共通処理フロー(§7)で全 API 共通に発生するエラーを定義する。各エラーの発生条件は §7 共通処理フローで表現する。各 API 文書はこれらを再掲せず「API-COM の共通エラー一覧(本書 §4.1)を参照」と記載する。

| ERR ID | エラー名 | HTTPステータス | 開発者向けメッセージ |
|---|---|---|---|
| ERR-001 | 認証失敗 | 401 | Unauthorized |
| ERR-002 | 権限なし | 403 | Forbidden |
| ERR-006 | バリデーションエラー | 400 | Validation failed |
...
flowchart TD
  ...
  Start --> P1 --> D1
  D1 -- "認証失敗" --> E1["ERR-001 を返却"] --> End
  D1 -- "認証成功" --> D2
  D2 -- "不認可" --> E2["ERR-002 を返却"] --> End
  D2 -- "認可" --> D3
  D3 -- "不正" --> E6["ERR-006 を返却"] --> End
  D3 -- "OK" --> Body --> End
```

## 共通処理詳細

共通処理フローの各処理((A)〜(D))で行う内容を、個別 API の §5 処理詳細と同じ形式で定義する。
...
- トークンが無い・不正な場合も含め、検証結果を (B) 認証判定へ渡す。
...
(A) JWT検証の結果が有効かを判定し、認証の成否を決める。トークン未指定・無効・期限切れの場合は ERR-001 を返す。
...
| No | 判定対象 | 条件 |
|---|---|---|
| 条件(1) | (A) JWT検証の結果.ロール | 当該 API の認可(各 API §1 基本情報)で許可される(CFR-002) |
...
共通処理フローで返却するエラーレスポンスの各項目に設定する値を定義する。封筒構造は §4 エラーレスポンスが正本。

| 項目名 | データ型 | 値 | 説明 |
|---|---|---|---|
| エラーコード | String | ERR-001((B) 認証判定)・ERR-002((C) 認可判定)・ERR-006((D) 入力バリデーション) | 返却するエラーコード |
| 開発者向けメッセージ | String | 返却する ERR の開発者向けメッセージ(§4.1 共通エラー一覧) | 返却する開発者向けメッセージ |
| エラー明細 | Array | ERR-006 は違反項目ごとの明細(field=違反項目・message=違反したルール内容)、ERR-001 / ERR-002 は空配列(明細なし) | 返却するエラー明細 |

## 入力バリデーションの範囲

(D) 入力バリデーションが検証するのは、リクエスト単体で機械的に判定できる構文的チェックに限る。
