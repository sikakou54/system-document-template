...
共通処理フローの各処理((A)〜(G))で行う内容を、個別 API の §5 処理詳細と同じ形式で定義する。
...
認証に失敗した(トークン未指定・無効・期限切れ)場合のエラーレスポンスを返却する。封筒構造は §4 エラーレスポンスが正本。

| 項目名 | データ型 | 値 | 説明 |
|---|---|---|---|
| エラーコード | String | ERR-001 | 返却するエラーコード |
...
認証主体のロールが当該 API に許可されていない場合のエラーレスポンスを返却する。封筒構造は §4 エラーレスポンスが正本。

| 項目名 | データ型 | 値 | 説明 |
|---|---|---|---|
| エラーコード | String | ERR-002 | 返却するエラーコード |
| 開発者向けメッセージ | String | 返却する ERR の開発者向けメッセージ(エラーメッセージ一覧.md) | 返却する開発者向けメッセージ |
| エラー明細 | Array | 空配列(明細なし) | 返却するエラー明細 |
...
リクエストが構文ルールを満たさない場合のエラーレスポンスを返却する。封筒構造は §4 エラーレスポンスが正本。

| 項目名 | データ型 | 値 | 説明 |
|---|---|---|---|
| エラーコード | String | ERR-006 | 返却するエラーコード |
| 開発者向けメッセージ | String | 返却する ERR の開発者向けメッセージ(エラーメッセージ一覧.md) | 返却する開発者向けメッセージ |
| エラー明細 | Array | 違反項目ごとの明細(field=違反項目・message=違反したルール内容) | 返却するエラー明細 |

## 入力バリデーションの範囲

(F) 入力バリデーションが検証するのは、リクエスト単体で機械的に判定できる構文的チェックに限る。
