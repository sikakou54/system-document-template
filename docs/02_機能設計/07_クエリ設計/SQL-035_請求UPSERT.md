...
| :status | 請求ステータス | INTEGER | Yes | 3=支払済 / 4=失敗 等(TBL-008/ENM-1) |
...
INSERT INTO T_INVOICES (
  ...
  :user_id, :stripe_invoice_id, :billing_period, :amount, :status   -- STATUS: 3=支払済 / 4=失敗 等(TBL-008/ENM-1)
)
...
| STATUS | 請求ステータス | INTEGER | 反映後の請求ステータス(TBL-008/ENM-1) |
...
| 想定件数 | 最大1件(1請求1行) |
| 注意点 | ON CONFLICT の対象は一意インデックス UX_INVOICES_STRIPE。Webhook 再送でも同じ結果に収束する(冪等)。区分値は TBL-008/ENM-1。UPDATED_AT はアプリで現在時刻を設定する |
