...
| 論理名 | 物理名 | 型 | 必須 | 説明・制約 |
|---|---|---|---|---|
| 署名 | Stripe-Signature | string | Yes | HTTPヘッダ。Stripe 署名検証に使用する |
| イベントID | id | string | Yes | Stripe イベントの一意なID。冪等キーとして使用 |
| イベント種別 | type | string | Yes | イベント種別(checkout.session.completed / customer.subscription.updated / customer.subscription.deleted / invoice.paid / invoice.payment_failed) |
| イベントデータ | data.object | object | Yes | イベント対象オブジェクト(session / subscription / invoice) |
...
| 論理名 | 物理名 | 型 | 説明 |
|---|---|---|---|
| 受信結果 | received | boolean | 受信・処理受付を示す。常に true を返し、Stripe に受信完了を通知する |
...
イベントIDが未処理の場合のみ、イベント種別に応じて課金契約状態・請求状態を更新する(冪等)。処理済みのイベントIDはスキップする。イベント種別ごとの処理は下表のとおり。Stripe 連携・DB 更新に失敗した場合はエラー(失敗)を返す。
...
入力バリデーションの構文ルールを、成立条件(AND / OR の論理式)で定義する。成立条件を満たさない場合、エラー列のコードを返し、違反項目ごとに details[] へ {field=物理名, message=メッセージ列} を設定する。署名の真正性の検証(署名値の検証)は §5 個別処理フロー((1) 署名検証・(2) 署名判定)に定義する。

| 論理名 | 物理名 | 成立条件 | エラー | メッセージ |
|---|---|---|---|---|
| 署名 | Stripe-Signature | 指定あり AND string | ERR-006 | Stripe-Signature ヘッダは必須です |
