...
| 初期ユーザー登録 | 本システムはアプリ上のユーザー登録(サインアップ)機能を持たない(スコープ外)。利用者(M_USERS)は、運用手順により LIB-001 で定義したハッシュ方式のパスワードを含むレコードを D1 へ投入(シード)して用意する。ログイン(UC-007)の前提条件はこの投入により満たす | CFR-001、TBL-001、LIB-001 |
