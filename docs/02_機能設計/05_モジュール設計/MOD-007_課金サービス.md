...
| getBillingUsage | 当月の利用量・請求見込みと請求履歴を取得する | ユーザーID:userId, ページ:page, 取得件数:limit | BillingUsage(当月利用量, 請求見込み, 請求履歴一覧) | - |
...
T_USAGE_RECORDS(TBL-007)から USER_ID 一致かつ当月分の記録の USAGE_MINUTES・AMOUNT を集計し、当月の利用量(合計利用時間)と請求見込み(合計金額。AMOUNT の合計)を求める。
...
|---|---|
| ユーザーID | 引数.userId |
| 対象月 | 現在の年月(Asia/Tokyo 基準) |

#### (2) 請求履歴取得

T_INVOICES(TBL-008)から USER_ID 一致の請求を BILLING_PERIOD 降順で取得し、page / limit でページネーション(API-COM §5)を適用する。当月利用量・請求見込みと請求履歴をまとめて返す。分岐・エラーはない。
...
| 論理名 | 物理名 | 設定値 |
|---|---|---|
| 課金利用状況 | BillingUsage | 当月利用量・請求見込み((1) の結果)と請求履歴一覧((2) の結果) |
