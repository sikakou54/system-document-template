...
sequenceDiagram
    ...
    CRON->>JOB: 定期起動(*/15 * * * *)
    JOB->>RES: リマインド対象抽出(STATUS=1 AND REMIND_STATUS=1 AND 開始30分以内)
    RES-->>JOB: 対象予約一覧
    alt 対象あり
        loop 対象予約ごと
            JOB->>Q: 対象予約をキューへ投入
            ...
            alt 送信成功
                RSD-->>MOD: 送信完了
                MOD->>RES: REMIND_STATUS=2(送信済)に更新
            else 送信失敗(5分間隔で最大3回リトライ後も失敗)
                RSD-->>MOD: 送信失敗
                MOD->>RES: REMIND_STATUS=3(失敗)に更新
                MOD->>RSD: 管理者(ROLE=2)へ失敗通知メール送信
            end
        end
    else 対象なし
        JOB-->>CRON: 正常終了
...
| 2 | JOB-001 → TBL-003 | 予約済(STATUS=1)・未送信(REMIND_STATUS=1)・開始30分以内の予約を抽出する |
...
| 6 | MOD-006 → TBL-003 | 送信成功は REMIND_STATUS=2、3回失敗は REMIND_STATUS=3 に更新する |
...
| メール送信失敗(3回リトライ上限) | MOD-006 が REMIND_STATUS=3(失敗)に更新し、管理者へ失敗を通知して次の予約へ継続する |
| 再実行(同一予約が再抽出) | REMIND_STATUS=1(未送信)のみ抽出するため二重送信しない |
