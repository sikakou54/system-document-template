...
MeetRoom の要件〜設計のトレース経路を対応表で一元管理する。
...
| ファイル | 対象経路 | 目的 |
|---|---|---|
| traceability_br_fr_uc.md | BR→FR/CFR→UC | 業務要件が機能要件・共通機能要件・ユースケースへ落ちているかを追跡する |
| traceability_uc_design.md | UC→SCR/API | 各ユースケースを実現する画面・API を追跡する |
| traceability_fr_nfr.md | FR→CFR/NFR | 機能要件に適用される共通機能要件・非機能要件と機能固有NFRの有無を追跡する |
