<!-- コピーして 02_機能設計/03_API設計/API-XXX_API名.md として使用。index.md への行追加を先に行うこと -->
<!-- 【フォーマット厳守】各表の列見出し(ヘッダ行)およびセクション構成(##/###/####)はテンプレート定義のまま変更しない。列の追加・削除・改名、見出しへの補足付記(例:「引数項目」→「引数項目(MOD-002 IF)」)、テンプレートに無いセクション・表・小見出しの追加は禁止。定義済みの表には行の追加と各セルの内容記入のみを行う(docs/CLAUDE.md §8-1・§10-4)。補足はセル内の値、または各セクションの説明文(段落)に記載する -->
...
- 説明: 項目の意味。
定義ルール:
- 論理名=日本語の表示名、物理名=レスポンス項目名(実装キー)。
- 配列・ネストは items[].xxx / equipments[].xxx の形で階層を表す。
-->
## 3. レスポンス
...
<!--
【5. 処理詳細】
定義内容: §4 処理フローの各処理((1)(2)…)について、呼び出すモジュール・引数・取得内容・条件分岐・レスポンス設定など具体的な処理内容を定義する。
...
- 取得結果を参照する箇所(設定値・判定対象など)は必ず「(x) 処理名の結果」の形で取得元ステップを明記する。
-->
## 5. 処理詳細
...
- 呼び出しモジュール表: MOD-ID=モジュールID／処理名=呼び出すメソッドの和名(呼び出しが無ければ「なし」)。
- 引数表: 引数項目=モジュールに渡す引数／値=渡す値(リクエスト項目や「(x) 処理名の結果」)。
定義ルール:
...
項目説明:
- 見出し直後の説明文: この判定で行う内容(1行)。
- #### 条件定義／#### 条件分岐マトリクス: 判定を構成する2つの表(各定義は直下のコメントを参照)。
- レスポンス設定表: レスポンスを返却する処理では、論理名／物理名=返す項目、設定値=設定する値(取得元を「(x) 処理名の結果」で明記)を末尾に付す。レスポンス返却以外の処理では「なし」とする。
定義ルール:
...
| 7 | 業務ルール | 重複不可、予約可能期間など、業務上守る条件を定義する | 個別フロー(本判定) |
-->
#### 条件定義
...
レスポンスを返却する処理では、各項目に設定する内容を定義する。レスポンス返却以外の処理では「なし」とする。
...
項目説明:
- 論理名: 対象項目の日本語表示名。
- 物理名: パラメータ名(details[].field に設定する)。
...
- 成立条件を満たさない場合のエラーは ERR-006。違反項目ごとに details[] へ {field=物理名, message=メッセージ列} を設定する。
- DB 参照・業務ルールを伴う判定(存在確認・重複など)はここに書かず §5 個別処理フローに定義する。
-->
## 6. バリデーション

入力バリデーションの構文ルールを、成立条件(AND / OR の論理式)で定義する。成立条件を満たさない場合、エラー列のコードを返し、違反項目ごとに details[] へ {field=物理名, message=メッセージ列} を設定する。任意項目は「指定なし OR(指定あり AND 制約)」の形で表す。DB 参照・業務ルールを伴う判定は §5 個別処理フローに定義する。
