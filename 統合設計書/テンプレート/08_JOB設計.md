...
<!-- 本節は統合設計書「8. JOB設計」の詳細テンプレート。Cloudflare Workers Paid、Cloudflare Queues、Cloudflare D1を前提とし、Cron→Queue→40件単位のconsumer処理を定義する。 -->
<!-- JOBはCloudflare D1、env.DB、D1 API、物理データ構造、SQL、M-006へ直接アクセスしない。データ処理はQueue consumerからM-002の公開IFへ委譲する。 -->
...
- Queuesの `queue()` consumerだけが、1 messageにつきM-002の指定公開IFをちょうど1回呼び出し、1チャンクを処理する。consumer自身は対象を反復せず、対象抽出・個別更新・D1原子実行をM-002以下へ委譲する。
...
- D1 Statement数はM-002以下で、`batch()`内の各Statement、状態/version再読込み、即時再試行を各1件として実測し、consumerへ論理結果として返す。consumerは900超過を契約違反として扱う。
...
- Queue consumer設定は `max_batch_size=1`、`max_concurrency=1`、`max_retries=3`、DLQ必須とする。handlerは成功したmessageだけをackし、失敗を握りつぶさない。
...
- JOB handlerへはQueue producerとM-002公開IFだけを注入し、`env.DB`、D1オブジェクト、SQL-ID、TBL-ID、M-006を渡さない。

## 8.2 JOB一覧

| JOB-ID | JOB名 | 目的 | 起動・継続経路 | 唯一の業務呼出先 |
|---|---|---|---|---|
| JOB-XXX | <JOB名> | <目的> | `scheduled()` / 手動 → 初回Queue投入 → `queue()` consumer → 継続Queue | M-002/IF-XX（consumerが1 messageにつき1回。scheduled/manualは呼ばない） |

<!-- 8.3のブロックをJOB-IDごとに複製する。処理フローの番号・名称は処理詳細と完全一致させる。 -->
## 8.3 JOB-XXX <JOB名>
...
| 項目 | 内容 |
|---|---|
| JOB-ID / JOB名 | JOB-XXX / <JOB名> |
| 目的 / トレース元 | <目的> / UC-XXX・§3.x |
| 本番基盤 | Cloudflare Workers Paid + Cloudflare D1 + Cloudflare Queues |
| Workers handler | `scheduled()`（初回message投入のみ） / `queue()`（チャンク実処理） / 認証済み手動入口（初回message投入のみ） |
...
| 呼出モジュール / 公開IF | M-002 社員管理アプリケーション / M-002/IF-XX `<チャンク処理名>` |
| 禁止事項 | JOBからM-006、D1、`env.DB`、D1 API、SQL、物理表へ直接アクセスしない |

### 8.3.2 起動パラメータ・継続message
