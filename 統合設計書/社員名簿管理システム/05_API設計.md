...
| 依存方向 | `画面 → Cloudflare Worker API境界 → 単一業務モジュール → M-006 → D1` とする。D1 Worker BindingはAPI境界へ渡さない |

### 5.1.1 API境界の責務・禁止事項

- API-001〜009およびAPI-011〜018は§5.2で完全修飾したM-002の公開IFだけを1回呼ぶ。API-010はM-003/IF-01だけを1回呼ぶ。
...
固定コード値の唯一の正本は§2.5とし、本表はAPI入出力で用いる表記の参照表である。APIは列挙外を`VALIDATION_ERROR`として受け付けず、M-002も永続化前に同じ列挙で再検証する。
...
固定ロールコード、操作権限、閲覧スコープの唯一の正本は§2.6とし、APIは次の認可結果をM-002から受け取る。複数ロール時は操作権限と許可項目の和集合、閲覧スコープは`ALL > ORGANIZATION > SELF`の優先順で決定し、有効ロールが0件の利用者はすべて拒否する。
...
| 4 | 検索レスポンス生成 | 境界 | 許可済み項目とページ情報を200へ変換する | - | (2)の成功結果 | §5.3.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 業務エラーを共通形式へ変換する | - | (2)のエラー、traceId | §5.3.7のHTTPエラー |
...
| retirementTypeCode | body | string | No | 退職区分。未設定または権限による非返却時は省略。返却時は§2.5の許可値 |
...
| `EMPLOYEE` | 対象社員IDが本人の場合だけ§5.4.3の全項目 |

§5.4.3の必須欄は、その項目が上表で返却許可された場合の必須性を表す。上表で許可されない項目はJSONプロパティ自体を省略し、NULLやマスク文字列へ置換しない。
...
| 4 | 詳細レスポンス生成 | 境界 | 許可済み社員詳細を200へ変換する | - | (2)の成功結果 | §5.4.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・権限等を共通形式へ変換する | - | (2)のエラー、traceId | §5.4.7のHTTPエラー |
...
| 6 | employmentTypeCode | 必須かつ§2.5（§5.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 4 | 登録レスポンス生成 | 境界 | 登録結果を201レスポンスへ変換する | - | (2)の登録社員 | §5.5.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 重複・マスター無効等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.5.7のHTTPエラー |
...
| 5 | employmentTypeCode | 指定時は§2.5（§5.1.3 API参照表）の許可値 | VALIDATION_ERROR |
...
| 4 | 更新レスポンス生成 | 境界 | 更新結果を200へ変換する | - | (2)の成功結果 | §5.6.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・重複・競合等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.6.7のHTTPエラー |
...
| 4 | 異動レスポンス生成 | 境界 | 新所属を201レスポンスへ変換する | - | (2)の成功結果 | §5.7.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・状態・期間・競合等を変換する | - | (2)のエラー、traceId | §5.7.7のHTTPエラー |
...
| 3 | retirementTypeCode | 指定時は§2.5（§5.1.3 API参照表）の許可値。省略時はNULL | VALIDATION_ERROR |
...
| 4 | 退職レスポンス生成 | 境界 | 即時/予定と更新後状態を200へ変換する | - | (2)の成功結果 | §5.8.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・日付・状態・競合等を変換する | - | (2)のエラー、traceId | §5.8.7のHTTPエラー |
...
| 3 | 取得結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 履歴レスポンス生成 | 境界 | M-002が実値・JSON本文を除外して表示文字列化したchangeSummaryを含む履歴とページ情報を200へ変換する。0件は空配列とする | - | (2)の成功結果 | §5.9.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限・不存在等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.9.7のHTTPエラー |
...
| 目的 | 認可範囲内から、指定基準日に§5.1.5の利用可能条件を満たす組織、または管理用の全組織を返す |
...
| 3 | 取得結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 一覧レスポンス生成 | 境界 | 組織一覧を200へ変換する | - | (2)の成功結果 | §5.10.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.10.7のHTTPエラー |
...
| 目的 | 指定基準日に§5.1.5の利用可能条件を満たす役職、または管理用の全役職を返す |
...
| 3 | 取得結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 一覧レスポンス生成 | 境界 | 役職一覧を200へ変換する | - | (2)の成功結果 | §5.11.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.11.7のHTTPエラー |
...
| 4 | ログインレスポンス生成 | 境界 | トークンと認証主体情報を200へ変換し、`Cache-Control: no-store`を付与する | - | (2)の成功結果 | §5.12.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 認証失敗・アカウント無効・外部障害を変換する | - | (2)のエラー、traceId | §5.12.7のHTTPエラー |
...
| 4 | 登録レスポンス生成 | 境界 | M-002が返した登録組織と`createdAt`を201レスポンスへ変換する | - | (2)の成功結果 | §5.13.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 重複・期間等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.13.7のHTTPエラー |
...
| 3 | 更新結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 更新レスポンス生成 | 境界 | M-002が返した更新後組織と`updatedAt`を200へ変換する | - | (2)の成功結果 | §5.14.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・重複・期間・競合等を変換する | - | (2)のエラー、traceId | §5.14.7のHTTPエラー |
...
| 3 | 登録結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 登録レスポンス生成 | 境界 | M-002が返した登録役職と`createdAt`を201レスポンスへ変換する | - | (2)の成功結果 | §5.15.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 重複・期間等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.15.7のHTTPエラー |
...
| 3 | 更新結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 更新レスポンス生成 | 境界 | M-002が返した更新後役職と`updatedAt`を200へ変換する | - | (2)の成功結果 | §5.16.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 不存在・重複・期間・競合等を変換する | - | (2)のエラー、traceId | §5.16.7のHTTPエラー |
...
| 3 | 取得結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | 一覧レスポンス生成 | 境界 | 有効ロール一覧を200へ変換する | - | (2)の成功結果 | §5.17.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.17.7のHTTPエラー |
...
|---|---|---|---|---|
| employeeId | path | string | Yes | 対象社員ID |
| effectiveOn | query | string(date) | No | 指定時点の割当を取得。未指定時は現在・将来を含む |
...
| No | 処理名 | 種別 | 処理詳細 | 呼出先公開IF・論理処理名 | 引数・参照値 | 結果変換 |
|---:|---|---|---|---|---|---|
| 1 | ロール取得要求生成 | 境界 | 認証主体、社員ID、基準日から論理要求を生成する | - | 認証主体、path、query、traceId | 社員ロール取得要求 |
| 2 | 社員ロール取得呼出 | モジュール呼出 | 管理権限、対象利用者存在、指定日時点または現在・将来の期間対象割当取得を委譲する。ロールマスターが現在`active=false`の既存割当も除外せず、`roles[].active=false`として受け取る | M-002/IF-12 ロール管理 | (1)の要求 | `active`を含むロール割当または業務エラー |
| 3 | 取得結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | ロールレスポンス生成 | 境界 | 利用者状態とロール割当を200へ変換する。モジュールから受け取った`roleActive`を`roles[].active`へそのまま設定し、falseの既存割当も参照可能とする | - | (2)の成功結果 | §5.18.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限・不存在等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.18.7のHTTPエラー |

### 5.18.7 エラー定義

| HTTP | エラーコード | 発生条件 | API境界の処理 |
|---:|---|---|---|
| 400 | VALIDATION_ERROR | 社員ID・基準日が不正 | 共通形式へ変換 |
| 401 | UNAUTHENTICATED | 認証情報が無効 | 共通形式へ変換 |
| 403 | FORBIDDEN | 権限管理権限がない | 共通形式へ変換 |
| 404 | EMPLOYEE_NOT_FOUND | 対象社員が存在しない | 共通形式へ変換 |
...
| 3 | 更新結果判定 | 判定 | (2)の結果を判定する | - | (2)の結果 | 成功は(4)、エラーは(5) |
| 4 | ロールレスポンス生成 | 境界 | 更新後割当を200へ変換する。変更ありは版数更新結果、完全同一割当は取得済み利用者アカウントの`updatedAt`を使用する | - | (2)の成功結果 | §5.19.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限・不存在・ロール無効・競合等を変換する | - | (2)のエラー、traceId | §5.19.7のHTTPエラー |
...
| fields | body | array(string) | No | §5.20.2.1の出力項目コード。省略時は標準5項目、指定時は1件以上で指定順を維持する |
...
| `employmentType` | 雇用区分 | employmentTypeCode | §2.5の表示名（正社員、契約社員等）へ変換 |
| `status` | 在籍状態 | status | §2.5の表示名（在籍中 / 退職）へ変換 |
...
| 2 | organizationId / positionId | 指定時はUUID形式 | VALIDATION_ERROR |
| 3 | fields | 指定時は配列、1件以上、§5.20.2.1の定義済み項目で重複なし。未知・重複は400 | VALIDATION_ERROR |
...
| 4 | ファイルレスポンス生成 | 境界 | M-002が§5.20.2.1の対応で生成した指定順ヘッダー・行をCSVまたはXLSXのファイル本体とHTTPヘッダーへ変換する | - | (2)の成功結果 | §5.20.3のレスポンス |
| 5 | エラーレスポンス生成 | 境界 | 権限・0件・上限超過等をHTTPエラーへ変換する | - | (2)のエラー、traceId | §5.20.7のHTTPエラー |
