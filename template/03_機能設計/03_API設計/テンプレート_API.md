...
<!-- 本 API の固有エラーは本文書 §7 にインライン定義する(ERR ID/エラー名/HTTPステータス/開発者向けメッセージ)。エラーの発生条件は §7 に持たせず、個別処理フロー(§4/§5)・共通処理フロー(API-COM §7)で表現する。認証・認可・バリデーション等の共通エラーは API-COM_共通設計.md に定義済みのため再記載せず ERR ID で参照する -->
...
- データを取得・検証・整形して、その結果を判定する場合は、判定の前に取得・検証・整形処理を独立したステップ(処理型)として定義し、判定はその結果を参照する。判定ノードの中で暗黙に取得・整形を行わない。これは個別処理フローに限らず、共通処理フロー(認証の JWT 検証など)にも徹底して適用する。
...
項目説明:
- ERR ID: エラーコード(ERR-XXX)。返却時に API-COM §4 エラーレスポンスの封筒項目 error.code(論理名: エラーコード)に設定する。
- エラー名: エラーの日本語名称。
- HTTPステータス: このエラーで返す HTTP ステータスコード(400 / 403 / 404 / 409 など)。
- 開発者向けメッセージ: 返却時に API-COM §4 エラーレスポンスの封筒項目 error.message(論理名: 開発者向けメッセージ)に設定する開発者向けの説明文。
定義ルール:
- 各列は API-COM §4 エラーレスポンスの封筒項目に対応させる(ERR ID→エラーコード、開発者向けメッセージ→開発者向けメッセージ)。封筒のフィールド名(物理名)は本表に記載しない。
- エラーの発生条件(どの処理・判定で発生するか)は本表に持たせない。個別処理フローで返す固有エラーの発生条件は §4/§5 のフロー・条件分岐マトリクスで、共通エラーの発生条件は API-COM §7 共通処理フローで表現する。
- 入力バリデーション(ERR-006)などの共通エラーは共通処理フローで返し、定義は API-COM_共通設計.md にあるため本文書に再記載せず ERR ID で参照する。
- 本 API の固有エラーは本文書にインライン定義する(ERR ID/エラー名/HTTPステータス/開発者向けメッセージ)。ERR ID の採番は全文書横断で最大値+1、欠番の再利用は禁止。
-->
# 7. エラー

本 API 固有のエラーを以下に定義する。各列は API-COM §4 エラーレスポンスの封筒項目に対応する(ERR ID=エラーコード、開発者向けメッセージ)。発生条件は §4/§5 個別処理フローで表現する。
