...
<!-- エラーコード(ERR-XXX)の定義は エラーメッセージ一覧.md が正本(システム全体を通した連番で一元管理)。本文書 §7 は本 API が返却するエラーコードの参照一覧のみを記載し、定義(エラー名/HTTPステータス/開発者向けメッセージ)を再掲しない。発生条件は §7 に持たせず、固有エラーは個別処理フロー(§4/§5)・共通エラーは共通処理フロー(API-COM §7)で表現する -->
...
- 処理ノードは、§5 処理詳細で他の定義単位を呼び出す(委譲する)処理 —— `| MOD-ID | 処理名 |`・`| SQL-ID | クエリ名 |`・`| 外部サービス | 処理名 |` のいずれかの表を持つ処理 —— を二重角括弧 [["(n) 処理名"]](関数呼び出しブロック=サブルーチン形状)で、それ以外の内部処理・「処理結果」ブロックを矩形 ["(n) 処理名"] で記す。判定ノードは {"(n) 判定名"}、エラーノード ["(n) XXXエラー"] は番号付き矩形(処理結果と同じ終端の結果ブロック)とする。
...
構成: 各処理を ## (n) 処理名 の見出しで展開する。処理には「処理型」(モジュール呼び出し・引数)・「判定型」(#### 条件定義 ＋ #### 条件分岐マトリクス)・「エラー型」(エラーレスポンス設定値表)があり、各見出し・各表の定義内容は直下のコメントを参照する。§4 のエラーノードは ## (n) XXXエラー 節としてフロー出現順の位置に展開し、返却するエラーレスポンスの設定値を定義する(集約した「エラー結果」ブロックは設けない)。
定義ルール(セクション共通):
...
- 個別処理フロー(§4)にエラーノード「(n) XXXエラー」がある場合、そのノードごとに ## (n) XXXエラー 節を(フロー出現順の位置に)配置し、返却するエラーレスポンスの各項目に設定する値を | 項目名 | データ型 | 値 | 説明 | の形式で定義する(集約した「エラー結果」ブロックは設けない)。エラーレスポンス封筒の構造は API-COM §4 が正本のため再記載せず、設定値のみを正本参照で定義する。1つのエラーノードでパターンにより複数の ERR-XXX を返す場合は、エラーコードの値に返却コードと対応するパターンを併記する(例: ERR-007(会議室なし) / ERR-010(利用停止))。共通処理フロー(認証・認可・入力バリデーション)で返すエラーはここに定義せず、個別処理フローで返すエラーのみ定義する(API-001 のログイン失敗のように共通エラーコードを個別フローで返す場合を含む)。エラーノードの名称は「XXXエラー」で表し、処理命名規則(取得/登録/更新/削除/判定処理)の例外とする(「処理結果」と同じ終端の結果ブロック)。
...
<!--
【(n) XXXエラー】(エラー型ステップ)
定義内容: §4 のエラーノード「(n) XXXエラー」に対応し、返却するエラーレスポンスの各項目に設定する値を定義する。成功時の「処理結果」と対をなす終端の結果ブロック(番号付きステップ)。
...
- 見出し直後の説明文: どの判定・処理の失敗時に返すエラーかを1行で記載する。
- 設定値表: 返却するエラーレスポンスの項目(論理名: エラーコード／開発者向けメッセージ／エラー明細)ごとに設定する値を | 項目名 | データ型 | 値 | 説明 | で定義する。
定義ルール:
- エラーレスポンス封筒の構造(項目構成・物理名)は API-COM §4 が正本のため再記載せず、論理名(エラーコード／開発者向けメッセージ／エラー明細)で各項目の設定値のみを定義する。
- エラーコードの値は、当該エラーノードへ遷移する経路(判定の条件分岐マトリクス・処理の分岐)で返す ERR-XXX を指す。1ノードでパターンにより複数の ERR-XXX を返す場合は、返却コードと対応するパターンを併記する(例: ERR-007(会議室なし) / ERR-010(利用停止) / ERR-004(過去日時))。
- 開発者向けメッセージは エラーメッセージ一覧.md を参照する。エラー明細を持たないエラーは「空配列(明細なし)」とする。
- エラーノードの名称は処理命名規則(取得/登録/更新/削除/判定処理)の例外とし「XXXエラー」で表す(「処理結果」と同じ終端の結果ブロック)。
-->
## (3) XXXエラー

XXX判定で XXX できなかった場合のエラーレスポンスを返却する。封筒構造は API-COM §4 エラーレスポンスが正本。

| 項目名 | データ型 | 値 | 説明 |
|---|---|---|---|
| エラーコード | String | ERR-XXX | 返却するエラーコード |
...
- エラーコード: 成立条件を満たさない場合に返すエラーコード(ERR-006)。ERR-XXX は エラーメッセージ一覧.md へのリンクとして記載する(定義は同ファイル)。
定義ルール:
- 成立条件は AND / OR の論理式で表す(記号 ＆/｜ は使わない)。
- 任意項目は「指定なし OR(指定あり AND 制約)」の形で表す。
- 成立条件を満たさない場合のエラーは ERR-006。違反項目ごとに details[] へ {field=項目名, message=違反した成立条件の内容} を設定する。
- DB 参照・業務ルールを伴う判定(存在確認・重複など)はここに書かず §5 個別処理フローに定義する。
-->
# 6. バリデーション

入力バリデーションの構文ルールを、成立条件(AND / OR の論理式)で定義する。成立条件を満たさない場合、エラーコードを返し、違反項目ごとに details[] へ {field=項目名, message=違反した成立条件の内容} を設定する。任意項目は「指定なし OR(指定あり AND 制約)」の形で表す。DB 参照・業務ルールを伴う判定は §5 個別処理フローに定義する。

| 項目名 | 成立条件 | エラーコード |
|---|---|---|
|  |  | [ERR-XXX](エラーメッセージ一覧.md) |

<!--
【7. エラー】
定義内容: この API が返却するエラー(共通・固有)のエラーコードを参照一覧として示す。定義(エラー名・HTTPステータス・開発者向けメッセージ)は エラーメッセージ一覧.md が正本のため本文書には再掲しない。
定義する条件: 全 API で必須。本 API が返却するエラーコードを列挙する。
項目説明:
- エラーコード: このエラーの識別コード(ERR-XXX)。定義は エラーメッセージ一覧.md を参照する。返却時に API-COM §4 エラーレスポンスの封筒項目 error.code に設定する。
- 区分: 共通(全 API 共通処理フローで発生。API-COM §7)/ 固有(本 API の個別処理フローで発生。§4/§5)。
- 発生箇所: そのエラーを返す処理。共通は「共通処理フロー(認証/認可/入力バリデーション)」、固有は「個別処理フロー(§4/§5)」。
定義ルール:
- 本節は返却するエラーコードの参照のみを記載し、エラー名・HTTPステータス・開発者向けメッセージは エラーメッセージ一覧.md にのみ持つ(再掲しない)。エラーコードの採番も エラーメッセージ一覧.md 側で行う(最大値+1、欠番の再利用は禁止)。
- エラーの発生条件(どの処理・判定で発生するか)は本表に持たせない。固有エラーは §4/§5 のフロー・条件分岐マトリクス、共通エラーは API-COM §7 共通処理フローで表現する。
- 本 API に固有エラーが無い場合は「本 API に固有エラーはない」と記載し、共通エラーのみを列挙する。認証・認可・入力バリデーションが発生しない API は、その旨(該当共通エラーが発生しないこと)を記載してよい。
-->
# 7. エラー

本 API が返却するエラーの一覧。定義(エラー名・HTTPステータス・開発者向けメッセージ)は エラーメッセージ一覧.md が正本のため再掲しない。発生条件は、共通エラー(区分=共通)は API-COM §7 共通処理フロー、固有エラー(区分=固有)は §4/§5 個別処理フローで表現する。

| エラーコード | 区分 | 発生箇所 |
|---|---|---|
| ERR-XXX | 共通 / 固有 | 共通処理フロー(認証/認可/入力バリデーション) / 個別処理フロー(§4/§5) |
