...
- 取得APIでは、フローチャートの末尾に「レスポンス返却」ブロックを配置し、返却する項目をレスポンス返却処理の表で定義する。
...
- 引数表: 引数項目=呼び出し先に渡す引数／値=渡す値(リクエスト項目や「(x) 処理名の結果」)。呼び出しモジュール表または LIB-ID 表を記載する場合にのみ記載する。
定義ルール:
...
- レスポンス設定表: レスポンスを返却する処理では、項目名=返す項目、設定値=設定する値(取得元を「(x) 処理名の結果」で明記)を末尾に付す。レスポンス返却以外の処理では「なし」とする。
定義ルール:
